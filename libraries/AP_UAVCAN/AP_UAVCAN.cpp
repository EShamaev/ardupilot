...
static void gnss_fix_cb(const uavcan::ReceivedDataStructure<uavcan::equipment::gnss::Fix>& msg)
{
    if (hal.can_mgr != nullptr) {
        ...
        if (ap_uavcan != nullptr) {
            ...
            if (state != nullptr) {
                const uint64_t posix_offset = UNIX_OFFSET_MSEC;
                ...
                } else if (msg.status == uavcan::equipment::gnss::Fix::STATUS_TIME_ONLY) {
                    state->status = AP_GPS::GPS_Status::NO_FIX;
                ...
                state->vertical_accuracy = (pos_cov[8] > 0) ? sqrtf(pos_cov[8]) : -1.0F;

                const float horizontal_pos_variance = MAX(pos_cov[0], pos_cov[4]);
                state->horizontal_accuracy = (horizontal_pos_variance > 0) ? sqrtf(horizontal_pos_variance) : -1.0F;
...
{
    if (hal.can_mgr != nullptr) {
        if (hal.can_mgr->is_initialized() == true && _initialized == false) {
            auto *node = get_node();
...
uavcan::Node<0> *AP_UAVCAN::get_node()
{
    if (_node == nullptr && get_can_driver() != nullptr) {
        //_node = new uavcan::Node<AP_UAVCAN::NodePoolSize>(*get_can_driver(), get_system_clock(), _node_allocator);
