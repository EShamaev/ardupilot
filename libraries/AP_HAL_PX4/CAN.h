...
#define UAVCAN_STM32_RX_QUEUE_SIZE 64
...
public:
    BusEvent(PX4CANDriver& can_driver);
    ~BusEvent();
...
class PX4CAN: public AP_HAL::CAN {
    ...
    virtual int16_t configureFilters(const uavcan::CanFilterConfig* filter_configs, uint16_t num_configs);
...
class PX4CANDriver: public AP_HAL::CANManager {
    BusEvent update_event_;
    PX4CAN if0_;
    PX4CAN if1_;
    uint8_t can_number_;
    ...
    static PX4CAN* ifaces[UAVCAN_STM32_NUM_IFACES];

public:
    PX4CANDriver() :
        update_event_(*this), if0_(bxcan::Can[0], update_event_, 0, UAVCAN_STM32_RX_QUEUE_SIZE), if1_(
            bxcan::Can[1], update_event_, 1, UAVCAN_STM32_RX_QUEUE_SIZE), can_number_(0), initialized_(
                false), p_uavcan(nullptr)
    {
        uavcan::StaticAssert<(UAVCAN_STM32_RX_QUEUE_SIZE <= PX4CAN::MaxRxQueueCapacity)>::check();
    }
    ...
    virtual uint8_t getNumIfaces() const
    {
        if (can_number_ >= 2) {
