...
static void handleTxInterrupt(uint8_t iface_index)
{
    if (iface_index < UAVCAN_STM32_NUM_IFACES) {
        ...
        PX4CAN* iface = ((PX4CANDriver*) hal.can_mgr)->getIface(iface_index);
...
PX4CAN* PX4CANDriver::ifaces[UAVCAN_STM32_NUM_IFACES] = { UAVCAN_NULLPTR, UAVCAN_NULLPTR };
    ...
    struct BsPair {
                ...
                uint16_t(1000 * (1 + bs1) / (1 + bs1 + bs2)))
        {
            if (!(bs1_bs2_sum > arg_bs1)) {
                if (AP_BoardConfig::get_can_debug() >= 1) {
                    printf("PX4CAN::computeTimings bs1_bs2_sum > arg_bs1\n\r");
                }
            }
        }
...
int16_t PX4CAN::send(const uavcan::CanFrame& frame, uavcan::MonotonicTime tx_deadline, uavcan::CanIOFlags flags)
{
    ...
    mb.TDHR = (uint32_t(frame.data[7]) << 24)
              ...
              | (uint32_t(frame.data[6]) << 16) | (uint32_t(frame.data[5]) << 8) | (uint32_t(frame.data[4]) << 0);
    mb.TDLR = (uint32_t(frame.data[3]) << 24)

              | (uint32_t(frame.data[2]) << 16) | (uint32_t(frame.data[1]) << 8) | (uint32_t(frame.data[0]) << 0);
...
int PX4CAN::init(const uint32_t bitrate, const OperatingMode mode)
{
    ...
    can_->BTR = ((timings.sjw & 3U) << 24) | ((timings.bs1 & 15U) << 16)

                | ((timings.bs2 & 7U) << 20) | (timings.prescaler & 1023U) | ((mode == SilentMode) ? bxcan::BTR_SILM : 0);
 ...
 * CanDriver
 */
uavcan::CanSelectMasks PX4CANDriver::makeSelectMasks(const uavcan::CanFrame* (&pending_tx)[uavcan::MaxCanIfaces]) const
{
...
bool PX4CANDriver::hasReadableInterfaces() const
{
    if (can_number_ >= 2) {
...
int16_t PX4CANDriver::select(uavcan::CanSelectMasks& inout_masks,
...
void PX4CANDriver::initOnce(uint8_t can_number)
{
    /*
     * CAN1, CAN2
     */
...
int PX4CANDriver::init(const uint32_t bitrate, const PX4CAN::OperatingMode mode, uint8_t can_number)
{
    int res = 0;
    ...
    if (AP_BoardConfig::get_can_debug() >= 2) {
        printf("PX4CANDriver::init Bitrate %lu mode %d\n\r", static_cast<unsigned long>(bitrate), static_cast<int>(mode));
    }
    ...
    if (!initialized_once) {
        ...
        if (AP_BoardConfig::get_can_debug() >= 2) {
            printf("PX4CANDriver::init First initialization\n\r");
        }
        initOnce(can_number);
    }
    ...
    if (can_number >= 1) {
        if (AP_BoardConfig::get_can_debug() >= 2) {
            printf("PX4CANDriver::init Initing iface 0...\n\r");
        }
        ...
        if (res < 0) {             // a typical race condition.
            if (AP_BoardConfig::get_can_debug() >= 2) {
                printf("PX4CANDriver::init Iface 0 init failed %i\n\r", res);
            }
...
#if UAVCAN_STM32_NUM_IFACES > 1
        if (AP_BoardConfig::get_can_debug() >= 2) {
            printf("PX4CANDriver::init Initing iface 1...\n\r");
        }
        ifaces[1] = &if1_;                          // Same thing here.
        res = if1_.init(bitrate, mode);
        if (res < 0) {
            if (AP_BoardConfig::get_can_debug() >= 2) {
                printf("PX4CANDriver::init Iface 1 init failed %i\n\r", res);
            }
    ...
    if (AP_BoardConfig::get_can_debug() >= 2) {
        printf("PX4CANDriver::init CAN drv init OK, res = %d\n\r", res);
    }

    return res;
}

PX4CAN* PX4CANDriver::getIface(uint8_t iface_index)
{
    if (iface_index < UAVCAN_STM32_NUM_IFACES) {
...
bool PX4CANDriver::begin(uint32_t bitrate, uint8_t can_number)
{
...
extern "C" {
    static int can1_irq(const int irq, void*)
    {
        if (irq == STM32_IRQ_CAN1TX) {
            ...
            AP_HAL::panic("can1_irq unhandled");
        }
        return 0;
    }
    ...
    static int can2_irq(const int irq, void*)
    {
        if (irq == STM32_IRQ_CAN2TX) {
            ...
            AP_HAL::panic("can2_irq unhandled");
        }
        return 0;
    }
