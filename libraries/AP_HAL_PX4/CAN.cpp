...
int PX4CANManager::init(const uint32_t bitrate, const PX4CAN::OperatingMode mode, uint8_t can_number)
{
    ...
    static bool initialized_once[CAN_STM32_NUM_IFACES] = { false };
    ...
    if (can_number < CAN_STM32_NUM_IFACES) {
        ...
        if (!initialized_once[can_number]) {
            ...
            _ifaces_in_to_out[_ifaces_num - 1] = can_number;
...
{
    // Find which internal interface corrsponds to required outside physical interface
    if (iface_index < CAN_STM32_NUM_IFACES) {
        if (_ifaces_out_to_in[iface_index] != 255) {
