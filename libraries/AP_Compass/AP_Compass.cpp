            ...
            if (hal.can_mgr[i] != nullptr) {
                ...
                do {
                    ...
                    if (uavcan != nullptr) {
                        ...
                        if (freemag != UINT8_MAX && (_backend_count < COMPASS_MAX_BACKEND) && (_compass_count < COMPASS_MAX_INSTANCES)) {
                            _backends[_backend_count] = new AP_Compass_UAVCAN(*this);
                            if (((AP_Compass_UAVCAN*) _backends[_backend_count])->register_uavcan_compass(i, freemag)) {
                                _backend_count++;

                                added = true;
                            } else {
                                delete _backends[_backend_count];
                            }
                        }
                    }
                } while (added);
