...
const AP_Param::GroupInfo AP_BoardConfig_CAN::var_info[] = {
    ...
    AP_SUBGROUPINFO(_var_info_can[0], "P1_", 1, AP_BoardConfig_CAN, AP_BoardConfig_CAN::CAN_var_info),
#endif

#if MAX_NUMBER_OF_CAN_INTERFACES > 1
    // @Group: P2_
    // @Path: ../AP_BoardConfig/canbus.cpp
    AP_SUBGROUPINFO(_var_info_can[1], "P2_", 2, AP_BoardConfig_CAN, AP_BoardConfig_CAN::CAN_var_info),
#endif

#if MAX_NUMBER_OF_CAN_INTERFACES > 2
    // @Group: P3_
    // @Path: ../AP_BoardConfig/canbus.cpp
    AP_SUBGROUPINFO(_var_info_can[2], "P3_", 3, AP_BoardConfig_CAN, AP_BoardConfig_CAN::CAN_var_info),
#endif

#if MAX_NUMBER_OF_CAN_DRIVERS > 0
    // @Group: D1_
    // @Path: ../AP_BoardConfig/canbus.cpp
    AP_SUBGROUPINFO(_var_info_can_protocol[0], "D1_", 4, AP_BoardConfig_CAN, AP_BoardConfig_CAN::CAN_protocol_var_info),
#endif

#if MAX_NUMBER_OF_CAN_DRIVERS > 1
    // @Group: D2_
    // @Path: ../AP_BoardConfig/canbus.cpp
    AP_SUBGROUPINFO(_var_info_can_protocol[1], "D2_", 5, AP_BoardConfig_CAN, AP_BoardConfig_CAN::CAN_protocol_var_info),
#endif

#if MAX_NUMBER_OF_CAN_DRIVERS > 2
    // @Group: D3_
    // @Path: ../AP_BoardConfig/canbus.cpp
    AP_SUBGROUPINFO(_var_info_can_protocol[2], "D3_", 6, AP_BoardConfig_CAN, AP_BoardConfig_CAN::CAN_protocol_var_info),
#endif
...
#if HAL_WITH_UAVCAN
int8_t AP_BoardConfig_CAN::_st_can_enable[MAX_NUMBER_OF_CAN_INTERFACES];
...
void AP_BoardConfig_CAN::init()
{
    ...
    {
        _st_can_enable[i] = (int8_t) _var_info_can[i]._can_enable;
...
void AP_BoardConfig_CAN::px4_setup_canbus(void)
{
    ...
    for (uint8_t i = 0; i < MAX_NUMBER_OF_CAN_INTERFACES; i++)
    {
        uint8_t drv_num = _var_info_can[i]._can_enable;
    ...
    if (initret) {
        for (uint8_t i = 0; i < MAX_NUMBER_OF_CAN_DRIVERS; i++) {
            if (hal.can_mgr[i] != nullptr) {
                ...
                hal.console->printf("can_mgr %d initialized well\n\r", i + 1);

                if (_var_info_can_protocol[i]._protocol_enable == UAVCAN_PROTOCOL_ENABLE) {
